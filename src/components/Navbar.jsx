  ...
  return (
    <nav className="navbar" aria-label="Main Navigation">
      <div className="navbar-container">
        {/* Brand Logo */}
        <Link to="/" className="navbar-logo-link" aria-label="Pillar Steel Home" onClick={closeMobileMenu}>
              ...
              return (
                <li key={label}>
                  {href.startsWith('/') && !href.includes('#') ? (
                    <Link to={href} className={`nav-link${isActive ? ' active' : ''}`}>{label}</Link>
                  ) : (
                    <a href={href} className={`nav-link${isActive ? ' active' : ''}`}>{label}</a>
                  )}
                </li>
              );
            })}
      ...
      <div className={`mobile-menu-drawer ${isMobileMenuOpen ? 'open' : ''}`}>
        ...
        <ul className="mobile-nav-links">
          {navLinks.map(({ label, href }) => (
            <li key={label}>
              {href.startsWith('/') && !href.includes('#') ? (
                <Link to={href} className="mobile-nav-link" onClick={closeMobileMenu}>{label}</Link>
              ) : (
                <a href={href} className="mobile-nav-link" onClick={closeMobileMenu}>{label}</a>
              )}
            </li>
          ))}
        </ul>
